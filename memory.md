...
**NEXT (decided 2026-06-10, NOT built): email-based password reset via Resend.** #2 admin "send reset invite" + #3 self-service "¿Olvidaste tu contraseña?" on Login. Chosen email provider = **Resend** (3rd-party API; needs RESEND_API_KEY in `ldk-quiz-api` Lambda env + ldk.lat domain verified in Resend via DNS). Build list: DB migration for a reset-token table (token hash, user_id, expires_at, used_at); Lambda routes `POST /auth/forgot`, `POST /auth/reset`, `POST /admin/users/:id/send-reset`; Resend email helper (fetch to api.resend.com); new frontend pages `/forgot` + `/reset?token=`, Login link, Admin button, App.tsx routes, api/auth client fns. Security-sensitive — plan before building; gated on user completing Resend setup + providing API key.
